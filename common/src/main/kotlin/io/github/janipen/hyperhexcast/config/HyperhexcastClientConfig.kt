package io.github.janipen.hyperhexcast.config
...
object HyperhexcastClientConfig {
    @JvmStatic
    ...
    fun init() {
        ...
        holder.registerSaveListener { _, config ->
            HyperhexcastServerConfig.holder.config = HyperhexcastServerConfig.GlobalConfig(config.server)
            InteractionResult.PASS
        }

        // when we leave a server, clear our local copy of its config
        ClientPlayerEvent.CLIENT_PLAYER_QUIT.register { _ ->
            HyperhexcastServerConfig.onSyncConfig(null)
        }
    }

    @Config(name = Hyperhexcast.MODID)
    class GlobalConfig : GlobalData() {
        @Category("server")
        @TransitiveObject
        val client = ClientConfig()

        // this should only be used inside of this class; use HyperhexcastServerConfig.config to access the server-side config in other code
        @Category("server")
        @TransitiveObject
        val server = HyperhexcastServerConfig.ServerConfig()
    }
